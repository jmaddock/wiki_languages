...
class File_Importer(object):
    def __init__(self,wiki_name,db_file_name=None):
        ...
        if not db_file_name:
            self.db_file_name = '%s%s.json' % (self.db_path,wiki_name)
            self.db_file = open(self.db_file_name,'a')
            self.db = None
        else:
            self.db_file = open(self.db_file_name,'r')
            self.db = self.import_from_file()
        ...
        while True:
            ...
            for i,page in enumerate(self.dh.dump):
                if page.namespace == 1 or page.namespace == 0:
                    result = json.dumps(self.create_document(page))
            ...
            basic.write_log('inserted %s documents' % self.inserted_count)

    def import_from_file(self):
        with open(self.db_file) as data_file:    
