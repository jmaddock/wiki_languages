...
def get_num_editors(infile_name):
    ...
    lang = infile_name.replace(config.ROOT_PROCESSED_DIR,'').replace(config.COMBINED_RAW_EDITS,'').replace('/','')
    # get the total number of editors
    num_editors_no_IP = len(df.loc[df['user_id'] != 'None']['user_id'].unique())
    num_editors_include_anon = len(df['user_text'].unique())
